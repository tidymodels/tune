...
- When calculating resampling estimates, we can now use a weighted mean
  ...
  this using the new
  [`add_resample_weights()`](https://tune.tidymodels.org/reference/add_resample_weights.md)
  function. See
  [`?calculate_resample_weights`](https://tune.tidymodels.org/reference/calculate_resample_weights.md)
...
- [`tune_bayes()`](https://tune.tidymodels.org/reference/tune_bayes.md)
...
- Fixed a bug in
  ...
  using
  [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md)
  objects ([\#1110](https://github.com/tidymodels/tune/issues/1110))

- In
  [`tune_grid()`](https://tune.tidymodels.org/reference/tune_grid.md), a
  bug was fixed that caused inefficiency where preprocessing steps were
  applied to data being predicted several times (redundantly). We now
  execute that operation once.
  ([\#1101](https://github.com/tidymodels/tune/issues/1101))

### Other Changes

- The developer functions
  [`val_class_and_single()`](https://tune.tidymodels.org/reference/empty_ellipses.md)
  and
  [`val_class_or_null()`](https://tune.tidymodels.org/reference/empty_ellipses.md)
  are now deprecated
  ([\#1161](https://github.com/tidymodels/tune/issues/1161)).

- [`check_metrics()`](https://tune.tidymodels.org/reference/empty_ellipses.md)
  is deprecated in favor of
  [`check_metrics_arg()`](https://tune.tidymodels.org/reference/choose_metric.md)
  ([\#1160](https://github.com/tidymodels/tune/issues/1160)).

- [`finalize_workflow_preprocessor()`](https://tune.tidymodels.org/reference/tune-internal-functions.md)
  is deprecated
...
- Prepends a period to the name, and exports the following functions.
  ...
  - `check_grid()` (e.g., now named
    [`.check_grid()`](https://tune.tidymodels.org/reference/empty_ellipses.md))
...
- A major rewrite/refactor of the underlying code that runs
  [`tune_grid()`](https://tune.tidymodels.org/reference/tune_grid.md).
...
- Introduced support for parallel processing with mirai in addition to
  ...
  [`?parallelism`](https://tune.tidymodels.org/reference/parallelism.md)
  to learn more
...
- Post-processing: new
  [`schedule_grid()`](https://tune.tidymodels.org/reference/schedule_grid.md)
...
- Added a `type` argument to
  [`collect_metrics()`](https://tune.tidymodels.org/reference/collect_predictions.md)
...
- Added a new function,
  [`compute_metrics()`](https://tune.tidymodels.org/reference/compute_metrics.md),
...
- A method for rsample’s
  ...
  [`fit_resamples()`](https://tune.tidymodels.org/reference/fit_resamples.md),
  `tune_*()`, and
  [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md).
...
- [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md) will
  now error when supplied a fitted workflow
...
- Metrics from apparent resamples are no longer included when estimating
  performance with
  [`estimate_tune_results()`](https://tune.tidymodels.org/reference/empty_ellipses.md)
...
- Improves documentation related to the hyperparameters associated with
  ...
  [`?collect_extracts`](https://tune.tidymodels.org/reference/collect_predictions.md)
  to learn more.

- `eval_time` and `eval_time_target` attribute was added to tune
  objects. There are also
  [`.get_tune_eval_times()`](https://tune.tidymodels.org/reference/tune_accessor.md)
  and
  [`.get_tune_eval_time_target()`](https://tune.tidymodels.org/reference/tune_accessor.md)
  functions.

- [`collect_predictions()`](https://tune.tidymodels.org/reference/collect_predictions.md)
...
- Several functions gained an `eval_time` argument for the evaluation
  ...
  [`autoplot.tune_results()`](https://tune.tidymodels.org/reference/autoplot.tune_results.md)
  and the developer-focused
  [`check_initial()`](https://tune.tidymodels.org/reference/empty_ellipses.md)
...
- Ellipses (…) are now used consistently in the package to require
  ...
  [`augment.tune_results()`](https://tune.tidymodels.org/reference/augment.tune_results.md),
  [`collect_predictions.tune_results()`](https://tune.tidymodels.org/reference/collect_predictions.md),
  [`collect_metrics.tune_results()`](https://tune.tidymodels.org/reference/collect_predictions.md),
  [`select_best.tune_results()`](https://tune.tidymodels.org/reference/show_best.md),
  [`show_best.tune_results()`](https://tune.tidymodels.org/reference/show_best.md),
  and the developer-focused
  [`estimate_tune_results()`](https://tune.tidymodels.org/reference/empty_ellipses.md),
  [`load_pkgs()`](https://tune.tidymodels.org/reference/load_pkgs.md),
  and
  [`encode_set()`](https://tune.tidymodels.org/reference/empty_ellipses.md).
  Several other functions that previously did not have ellipses in their
  signatures gained them: this applies to
  [`conf_mat_resampled()`](https://tune.tidymodels.org/reference/conf_mat_resampled.md)
  and the developer-focused
  [`check_workflow()`](https://tune.tidymodels.org/reference/empty_ellipses.md).
...
- [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md) now
  works with the 3-way validation split objects from
  [`rsample::initial_validation_split()`](https://rsample.tidymodels.org/reference/initial_validation_split.html).
  [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md) and
  [`fit_best()`](https://tune.tidymodels.org/reference/fit_best.md) now
  have a new argument `add_validation_set` to include or exclude the
...
- Introduced a new function
  [`fit_best()`](https://tune.tidymodels.org/reference/fit_best.md) that
  provides a shorthand interface to fit a final model after parameter
  tuning. ([\#586](https://github.com/tidymodels/tune/issues/586))
...
- Introduced
  ...
  [`collect_notes()`](https://tune.tidymodels.org/reference/collect_predictions.md),
...
- Fixed bug in
  [`select_by_pct_loss()`](https://tune.tidymodels.org/reference/show_best.md)
...
- Extended
  [`show_best()`](https://tune.tidymodels.org/reference/show_best.md),
  [`select_best()`](https://tune.tidymodels.org/reference/show_best.md),
...
- [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md),
  ...
  [`control_grid()`](https://tune.tidymodels.org/reference/control_grid.md)
  to
  [`tune_bayes()`](https://tune.tidymodels.org/reference/tune_bayes.md)
  will fail but passing
  [`control_bayes()`](https://tune.tidymodels.org/reference/control_bayes.md)
  to [`tune_grid()`](https://tune.tidymodels.org/reference/tune_grid.md)
...
- The
  ...
  [`tune_grid()`](https://tune.tidymodels.org/reference/tune_grid.md) to
  control its verbosity.

- The
  [`control_last_fit()`](https://tune.tidymodels.org/reference/control_last_fit.md)
...
- [`show_notes()`](https://tune.tidymodels.org/reference/show_notes.md)
...
- [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md) now
  accepts a `control` argument to allow users to control aspects of the
  last fitting process via
  [`control_last_fit()`](https://tune.tidymodels.org/reference/control_last_fit.md)
...
- A check was added to
  [`fit_resamples()`](https://tune.tidymodels.org/reference/fit_resamples.md)
  and [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md)
  to give a more informative error message when a preprocessor or model
  have parameters marked for tuning.

- [`outcome_names()`](https://tune.tidymodels.org/reference/outcome_names.md)
...
- Fixed an issue in
  [`finalize_recipe()`](https://tune.tidymodels.org/reference/finalize_model.md)
...
- [`last_fit()`](https://tune.tidymodels.org/reference/last_fit.md) no
  longer accidentally adjusts the random seed
...
- Formatting of some messages created by
  ...
  [`message_wrap()`](https://tune.tidymodels.org/reference/message_wrap.md)
  function.
...
- In other plotting news,
  [`coord_obs_pred()`](https://tune.tidymodels.org/reference/coord_obs_pred.md)
...
- [`filter_parameters()`](https://tune.tidymodels.org/reference/filter_parameters.md)
...
- Added a `save_workflow` argument to `control_*` functions that will
  ...
  [`.get_tune_workflow()`](https://tune.tidymodels.org/reference/tune_accessor.md)
...
- [`select_best()`](https://tune.tidymodels.org/reference/show_best.md),
  [`select_by_one_std_err()`](https://tune.tidymodels.org/reference/show_best.md),
  and
  [`select_by_pct_loss()`](https://tune.tidymodels.org/reference/show_best.md)
