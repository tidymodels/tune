...
# ------------------------------------------------------------------------------
# tuning schedules for grid search
...
test_that("grid processing sschedule - model only, submodels", {
  ...
  reg_n <- length(sched_bst$second_stage)
  for (i in 1:reg_n) {
  ...
  irreg_n <- length(sched_sfd_bst$second_stage)
  expect_equal(irreg_n, 1L)

  expect_named(sched_sfd_bst$second_stage[[1]], c("min_n", "predict_stage", "trees"))
  expect_equal(
    sched_sfd_bst$second_stage[[1]] %>%
  ...
  expect_named(sched_odd_bst, c("second_stage"))
  expect_equal(nrow(sched_odd_bst), 1L)

  odd_n <- length(sched_odd_bst$second_stage)
  expect_equal(odd_n, 1L)

  expect_named(sched_odd_bst$second_stage[[1]], c("min_n", "predict_stage", "trees"))
  expect_equal(
    sched_odd_bst$second_stage[[1]] %>%
      dplyr::select(-predict_stage) %>%
      dplyr::select(trees, min_n),
    tibble(trees = c(2, 1, 2, 1, 2), min_n = c(1, 2, 3, 4, 5))
  )

  for (i in 1:nrow(sched_odd_bst$second_stage[[1]])) {
    prd <- sched_odd_bst$second_stage[[1]]$predict_stage[[i]]
    if (sched_odd_bst$second_stage[[1]]$min_n[i] == 1) {
  ...
  expect_named(sched_1_pt, c("second_stage"))
  expect_equal(nrow(sched_1_pt), 1L)
  expect_equal(length(sched_1_pt$second_stage), 1L)
  expect_named(
    sched_1_pt$second_stage[[1]],
    c("min_n", "predict_stage", "trees")
  )

  expect_equal(
    length(sched_1_pt$second_stage[[1]]$predict_stage),
    1L
  )
  expect_named(
    sched_1_pt$second_stage[[1]]$predict_stage[[1]],
    c("trees", "post_stage")
  )

  expect_equal(
    length(sched_1_pt$second_stage[[1]]$predict_stage[[1]]$post_stage),
    1L
  )
  expect_equal(
    dim(sched_1_pt$second_stage[[1]]$predict_stage[[1]]$post_stage[[1]]),
    1:0
  )
})
