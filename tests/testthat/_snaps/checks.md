...
# initial values
    ...
    Code
      tune::check_initial(data.frame(), extract_parameter_set_dials(wflow_1), wflow_1,
      mtfolds, yardstick::metric_set(yardstick::rsq), control_bayes())
    Condition
...
# check parameter finalization
    ...
    Code
      expect_error(p1 <- check_parameters(w1, data = mtcars, grid_names = character(0)),
      regex = NA)
    Message
...
---
    ...
    Code
      expect_error(p2 <- check_parameters(w2, data = mtcars), regex = NA)
    Message
      i Creating pre-processing data to finalize unknown parameter: mtry
...
---
    ...
    Code
      expect_error(p3_a <- check_parameters(w3, data = mtcars), regex = NA)
    Message
      i Creating pre-processing data to finalize unknown parameter: mtry
