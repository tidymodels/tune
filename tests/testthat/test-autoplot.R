...
test_that("two quantitative predictor marginal plot",{
  ...
  expect_equal(get_expr(p$mapping$x), expr(value))
  expect_equal(get_expr(p$mapping$y), expr(mean))
...
test_that("performance plot for iterative search",{
  ...
  expect_equal(get_expr(p$mapping$x), expr(.iter))
  expect_equal(get_expr(p$mapping$y), expr(mean))
...
test_that("parameter plot for iterative search",{
  ...
  expect_equal(get_expr(p$mapping$x), expr(.iter))
  expect_equal(get_expr(p$mapping$y), expr(value))
