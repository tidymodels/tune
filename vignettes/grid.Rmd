...
```{r grid}
set.seed(35)
grid_form <- tune_grid(Class ~ ., model = svm_mod, resamples = iono_rs, metrics = roc_vals, control = ctrl)
grid_form
```
...
```{r recipe}
set.seed(325)
rec_form <- tune_grid(iono_rec, model = svm_mod, resamples = iono_rs, metrics = roc_vals, control = ctrl)
rec_form
```
