...
```{r sorted-metrics}
show_best(grid_form, metric = "roc_auc")
```

## Executing the Grid Using a Recipe

The same syntax is used but a recipe is passed in to preprocess the data for modeling:
...
```{r best-rec}
show_best(rec_form, metric = "roc_auc")
```
