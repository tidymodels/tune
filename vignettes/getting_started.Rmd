...
The `tune` package helps optimize the modeling process. Users can _tag_ arguments in recipes and model objects for optimization. The search routines in `tune` can discover these arguments and test candidate values until a combination with good performance is found. 
...
However, based on the plot above, the potential non-linear relationship between the sale price and the might be different. The recipe above would constrain the nonlinearity of the predictors to be the same. We can probably do better than that. 

Individual `step_ns()` terms can be added to the recipe for each predictor. However, we want these to be identifiable; using the same syntax as above, we can't tell the difference between the two `deg_free` parameters. 
...
The `dials` package has a function called `param_set()` that can detect and collect the parameters flagged for tuning:
...
Grid search uses a pre-defined set of candidate parameters and evaluates these using resampling. To make the grid, a data frame is needed with column names matching the "id" column above. You could use one of the `dials` functions `grid_*`:
...
Alternately, `expand.grid()` also works:
...
There are two other ingredients that are needed. First, is a model specification. Using `parsnip`, a basic linear model can be used:
...
Using these objects, `tune_grid()` can be used:

```{r grid-search}
ames_res <- tune_grid(ames_rec, model = lm_mod, rs = cv_splits, grid = spline_grid)
```

The object is similar to the `rsample` object but with some extra columns:
...
Interestingly, latitude does _not_ like any degrees of freedom less than 8. How nonlinear are the optimal degrees of freedom? Let's plot these along with a value that did poorly: 
...
Looking at these plots, the smaller degrees of freedom are clearly under-fitting. Visually, the more complex splines might indicate that there is overfitting but this would results in poor RMSE values when computed on the hold-out data. 
